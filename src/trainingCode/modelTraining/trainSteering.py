...
labledSteeringFramesPath = steeringData.labledSteeringFramesPath

modelPath = 'Models/steeringModel.torch'
...
def train():
    ...
    for file in [f for f in listdir(labledSteeringFramesPath) if isfile(join(labledSteeringFramesPath, f))]:
        angle = file.split('_')[1]
        dataFiles.append(labledSteeringFramesPath + "/" + file)
        dataAngles.append(angle)

    if len(dataFiles) != len(dataAngles):
        raise Exception('Length of file and angle arrays not equal')
    
    print('Number of images in dataset: ', len(dataFiles))
